...
const Add = ({ isOpen, onClose, onCreated }) => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            console.log('Tạo thành công Employee Tax:', res.data)
            toast.success('Đã tạo thông tin thuế nhân viên!')
            onCreated?.()
            onClose()
        } catch (err) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} position="center">
            <div className="bg-white rounded-md p-6 max-w-2xl w-2xl">
                <h2 className="text-lg font-semibold mb-4">Thêm Thuế Nhân viên</h2>
                        ...
                        <div className='flex flex-col space-y-5 w-full'>
                            ...
                            <div>
                                <Input
                                    ...
                                    type="number"
                                    isRequired
                                    {...register('number_of_dependents', { required: 'Vui lòng nhập số người phụ thuộc' })}
                                />
                                {errors.number_of_dependents && <p className="text-sm text-red-500">{errors.number_of_dependents.message}</p>}
                            </div>
                        </div>
                    </div>
